  ...
  Footer(
    ...
    Div({ tw("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col sm:flex-row gap-10 sm:gap-20") }) {
      Nav({ tw("flex flex-col gap-2") }) {
        ...
        A(attrs = { href("#components"); tw("link link-hover text-base-content/60") }) { Text("Components") }
        A(attrs = { href("#themes"); tw("link link-hover text-base-content/60") }) { Text("Themes") }
        A(attrs = { href("https://github.com/mahyarmlk/compose-daisy-ui#readme"); tw("link link-hover text-base-content/60") }) { Text("Documentation") }
      }
      Nav({ tw("flex flex-col gap-2") }) {
        Span({ tw("footer-title opacity-100") }) { Text("Resources") }
        A(attrs = { href("https://github.com/mahyarmlk/compose-daisy-ui"); tw("link link-hover text-base-content/60") }) { Text("GitHub") }
        A(attrs = { href("https://github.com/mahyarmlk/compose-daisy-ui/blob/main/CHANGELOG.md"); tw("link link-hover text-base-content/60") }) { Text("Changelog") }
        A(attrs = { href("https://central.sonatype.com/artifact/io.github.mahyarmlk/compose-daisyui-js"); tw("link link-hover text-base-content/60") }) { Text("Maven Central") }
      }
    }
